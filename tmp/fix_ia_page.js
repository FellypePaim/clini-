...
const target = path.join(__dirname, '..', 'src', 'pages', 'SuperAdmin', 'SuperIAPage.tsx');
let content = fs.readFileSync(target, 'utf8');

const startMarker = '{actions.map((item) => (';
const index = content.indexOf(startMarker);
if (index === -1) {
    console.error('Marker not found: actions.map');
    // Log the current content length or something
    process.exit(1);
}
const endMarker = '</tbody>';
const nextTbody = content.indexOf(endMarker, index);
if (nextTbody === -1) {
    console.error('End marker (tbody) not found');
    process.exit(1);
}
const toReplace = content.substring(index, nextTbody);
...
const finalContent = content.substring(0, index) + replacement + content.substring(nextTbody);
