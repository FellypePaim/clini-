        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-2xl font-black text-[var(--color-text-primary)] uppercase">Inadimplência</h1>
            </div>
          </div>
      ...
      <main className="p-6 max-w-[1400px] mx-auto w-full space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className={`p-6 rounded-3xl border shadow-sm relative overflow-hidden flex flex-col justify-between h-[160px] ${kpi.dark ? `${kpi.darkBg} text-white` : 'bg-[var(--color-bg-card)] border-[var(--color-border)]'}`}>
              <div className="flex items-center justify-between">
                <span className={`text-[10px] font-black uppercase tracking-[0.15em] ${kpi.dark ? 'text-rose-200' : 'text-[var(--color-text-muted)]'}`}>{kpi.label}</span>
        ...
        <div className="bg-[var(--color-bg-card)] p-8 rounded-[32px] border border-[var(--color-border)] shadow-sm">
          <div className="mb-8 flex justify-between items-center">
            <div>
              <h3 className="text-sm font-black text-[var(--color-text-primary)] uppercase tracking-widest">Aging — Dias em Atraso</h3>
              <p className="text-[11px] text-[var(--color-text-muted)] font-bold mt-1 uppercase tracking-tight">Segmentação das pendências por tempo de atraso</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-50">
              {agingBuckets.map((b) => (
                <div key={b.faixa} className="text-center">
                  <p className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{b.faixa}</p>
                  <p className="text-lg font-black text-[var(--color-text-primary)] mt-1">{b.quantidade}</p>
        ...
        <div className="bg-[var(--color-bg-card)] rounded-[32px] border border-[var(--color-border)] shadow-sm overflow-hidden">
          <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
            <h3 className="text-sm font-black text-[var(--color-text-primary)] uppercase tracking-widest">Lançamentos Pendentes</h3>
              ...
              <tbody className="divide-y divide-slate-50 font-bold">
                {isLoading ? (
                  ...
                  <tr key={row.id} className="hover:bg-[var(--color-bg-card-hover)]/50 transition-colors group">
                    <td className="px-8 py-4 text-[var(--color-text-primary)] text-sm group-hover:text-rose-600 transition-colors">{row.descricao}</td>
                    <td className="px-8 py-4 text-[var(--color-text-secondary)] text-xs">{row.paciente_nome || '—'}</td>
                    <td className="px-8 py-4">
                      <span className="text-[10px] text-[var(--color-text-muted)] bg-[var(--color-bg-card)] px-2 py-1 rounded-lg">{row.categoria}</span>
                    </td>
                    <td className="px-8 py-4 text-center font-mono text-[11px] text-[var(--color-text-muted)]">
