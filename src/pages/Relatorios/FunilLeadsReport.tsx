        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-2xl font-black text-[var(--color-text-primary)] uppercase">Funil de Conversão Leads</h1>
            </div>
          </div>
      ...
      <main className="p-6 max-w-[1400px] mx-auto w-full space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className="bg-[var(--color-bg-card)] p-6 rounded-3xl border border-[var(--color-border)] shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{kpi.label}</span>
                <div className={`p-2.5 rounded-xl bg-[var(--color-bg-deep)] text-[var(--color-text-secondary)]`}>{kpi.icon}</div>
              </div>
              <span className="text-3xl font-black text-[var(--color-text-primary)] block">{kpi.valor}</span>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 bg-[var(--color-bg-card)] p-8 rounded-[32px] border border-[var(--color-border)] shadow-sm">
             <h3 className="text-sm font-black text-[var(--color-text-primary)] uppercase tracking-widest mb-8">Fluxo de Maturação (Leads)</h3>
             
             <div className="space-y-4">
               {isLoading ? (
                  <div className="h-[400px] animate-pulse bg-[var(--color-bg-deep)] rounded-3xl" />
                   ...
                   <div key={step.stage} className="relative">
                      ...
                      >
                        <div className="flex items-center gap-3">
                           <span className="w-8 h-8 rounded-full bg-[var(--color-bg-card)] flex items-center justify-center text-xs font-black shadow-sm" style={{ color: step.color }}>{idx + 1}</span>
                           <span className="text-xs font-black text-[var(--color-text-secondary)] uppercase tracking-widest">{step.stage}</span>
                        </div>
                        <div className="text-right">
                           <span className="text-sm font-black text-[var(--color-text-primary)]">{step.count} leads</span>
                           <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-tight">{percentage.toFixed(0)}% do total</p>
                        </div>
                      </div>
                      {idx < 3 && (
          ...
          <div className="lg:col-span-2 bg-[var(--color-bg-card)] p-8 rounded-[32px] border border-[var(--color-border)] shadow-sm flex flex-col">
             <h3 className="text-sm font-black text-[var(--color-text-primary)] uppercase tracking-widest mb-8">Volume Financeiro (R$)</h3>
             
             <div className="flex-1">
                {isLoading ? (
                  <div className="h-full animate-pulse bg-[var(--color-bg-deep)] rounded-3xl" />
                ) : (
                ...
                {data.filter(d => d.stage !== 'Perdido').map(step => (
                  <div key={step.stage} className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="font-bold text-[var(--color-text-muted)] uppercase tracking-tight">{step.stage}</span>
                    </div>
                    <span className="font-black text-[var(--color-text-primary)]">{step.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                  </div>
                ))}
             </div>
          </div>
