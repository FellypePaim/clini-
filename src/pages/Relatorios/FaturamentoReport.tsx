        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-2xl font-black text-[var(--color-text-primary)] uppercase">Faturamento por Período</h1>
            </div>
          </div>
          
          <div className="flex gap-2">
            <button onClick={exportPDF} disabled={isLoading || lancamentos.length === 0} className="flex items-center gap-2 px-6 py-3 bg-[var(--color-bg-card)] font-black text-[11px] uppercase tracking-widest text-[var(--color-text-secondary)] rounded-2xl hover:bg-[var(--color-border)] transition-all border border-[var(--color-border)] disabled:opacity-50">
      ...
      <main className="p-6 max-w-[1400px] mx-auto w-full space-y-6">
        ...
        <div className="bg-[var(--color-bg-card)] p-8 rounded-[32px] border border-[var(--color-border)] shadow-sm">
          <div className="mb-8 flex justify-between items-center">
             <div>
               <h3 className="text-sm font-black text-[var(--color-text-primary)] uppercase tracking-widest">Evolução de Receitas</h3>
               <p className="text-[11px] text-[var(--color-text-muted)] font-bold mt-1 uppercase tracking-tight">Consolidação diária de pagamentos recebidos</p>
             </div>
             <div className="flex gap-4">
               <div className="flex items-center gap-2">
                 <div className="w-3 h-3 rounded-full bg-emerald-500" />
                 <span className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">Realizado</span>
               </div>
             </div>
          </div>
          ...
          <div className="h-[360px] w-full">
            {isLoading ? (
               ...
               <div className="h-full flex flex-col items-center justify-center text-[var(--color-text-dim)] border-2 border-dashed border-slate-50 rounded-3xl">
                 ...
                 <p className="font-black text-[var(--color-text-muted)] uppercase tracking-widest text-xs">Nenhum dado financeiro consolidado</p>
               </div>
            ) : (
        ...
        <div className="bg-[var(--color-bg-card)] rounded-[32px] border border-[var(--color-border)] shadow-sm overflow-hidden">
          <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
            <h3 className="text-sm font-black text-[var(--color-text-primary)] uppercase tracking-widest">Principais Movimentações</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50 font-bold">
                {isLoading ? (
                  <tr><td colSpan={6} className="p-8 text-center text-[var(--color-text-dim)]">Carregando...</td></tr>
                ) : lancamentos.length === 0 ? (
                  <tr><td colSpan={6} className="p-12 text-center text-[var(--color-text-dim)] uppercase tracking-widest text-xs">Vazio</td></tr>
                ) : lancamentos.map((row) => (
                  <tr key={row.id} className="hover:bg-[var(--color-bg-card-hover)]/50 transition-colors group">
                    ...
                    <td className="px-8 py-4 text-[var(--color-text-primary)] text-sm group-hover:text-indigo-600 transition-colors">{row.descricao}</td>
                    <td className="px-8 py-4">
                      <span className="text-[10px] text-[var(--color-text-muted)] bg-[var(--color-bg-card)] px-2 py-1 rounded-lg">
                        {row.categoria}
                      </span>
                    </td>
                    <td className="px-8 py-4">
                      <div className="flex flex-col">
                        <span className="text-xs text-[var(--color-text-secondary)]">{row.paciente_nome || '—'}</span>
                        <span className="text-[9px] text-[var(--color-text-muted)] font-bold uppercase tracking-tight">{row.profissional_nome || 'Clínica'}</span>
                      </div>
                    </td>
