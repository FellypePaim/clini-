    ...
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--color-text-muted)] font-medium">Controle de consumo, custos e performance das redes neurais (Gemini Pro).</p>
        </div>
        <div className="flex items-center gap-3">
           ...
           <a
             ...
             className="p-3 bg-slate-800/40 border border-slate-700/50 text-[var(--color-text-muted)] hover:text-white rounded-xl transition-all">
              <Settings2 size={20} />
           </a>
        </div>
      </div>
          ...
          return (
            <div key={kpi.label} className="bg-slate-800/40 border border-slate-700/50 p-6 rounded-[32px] group relative overflow-hidden transition-all hover:bg-slate-800/50 shadow-xl">
               <div className="relative z-10">
                  ...
                  <div>
                     <p className="text-[10px] font-black uppercase text-[var(--color-text-muted)] tracking-[0.2em] mb-1">{kpi.label}</p>
                       ...
                       <tr key={item.action} className="hover:bg-slate-800/40 transition-colors group">
                          <td className="px-8 py-5">
                             <div className="flex items-center gap-4">
                                <div className="p-3 bg-slate-900/50 rounded-xl text-[var(--color-text-muted)] group-hover:text-purple-400 transition-colors">
                          ...
                          <td className="px-8 py-5 font-bold text-[var(--color-text-muted)]">{item.calls}</td>
                          <td className="px-8 py-5 font-bold text-[var(--color-text-muted)]">{item.tokens}</td>
                 ...
                 {(data?.topClinics || []).slice(0, 3).map((c: any, idx: number) => {
                   ...
                   <div key={c.nome || c.name || idx} className="bg-slate-800/20 border border-slate-700/30 p-5 rounded-3xl relative overflow-hidden group">
                      <div className="relative z-10">
                         ...
                         <p className="text-lg font-black text-[var(--color-text-dim)] mb-3">{c.tokens?.toLocaleString() || '0'} tokens</p>
                         <div className="flex justify-between text-[9px] font-black text-[var(--color-text-secondary)] uppercase mb-1">
                      ...
                      <div className="absolute top-0 right-0 p-3 text-[var(--color-text-primary)] group-hover:text-[var(--color-text-secondary)] transition-colors">
                         <TrendingUp size={32} />
                      </div>
                   </div>
                 )})}
        ...
        <div className="space-y-8">
           ...
           <div className="bg-slate-950/80 border border-slate-800/50 rounded-[40px] p-8 shadow-2xl font-mono relative overflow-hidden min-h-[400px]">
              <div className="flex items-center justify-between mb-6">
                 ...
                 <span className="text-[10px] font-bold text-[var(--color-text-secondary)] uppercase tracking-widest">IA LIVE CONSOLE</span>
              </div>
              ...
              <div className="space-y-4 text-[10px]">
                 <div className="flex gap-3">
                    ...
                    <span className="text-[var(--color-text-dim)]">detect_intent: "como marcar consulta?" &rarr; intent: APPOINTMENT_REQUEST</span>
                 </div>
                 <div className="flex gap-3">
                 ...
                 <div className="flex gap-3">
                    ...
                    <span className="text-[var(--color-text-dim)]">latency_alert: scribe_3.5 took 2450ms (p95 threshold exceeded)</span>
                 </div>
                 <div className="flex gap-3">
                    <span className="text-emerald-500 shrink-0">11:16:15</span>
                    <span className="text-[var(--color-text-muted)]">[INFO]</span>
                    <span className="text-[var(--color-text-dim)]">transcribe_audio: success (3MB audio) duration: 12.5s</span>
                 </div>
           ...
           <div className="bg-red-500/5 border border-red-500/10 p-8 rounded-[40px] space-y-6">
              ...
              <a
                href="/superadmin/configuracoes"
                className="block w-full py-4 bg-slate-800 text-[var(--color-text-dim)] hover:text-white font-black text-[10px] uppercase tracking-widest rounded-2xl transition-all border border-slate-700 hover:border-slate-500 text-center">
