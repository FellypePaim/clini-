...
export function CampanhasPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50/50 overflow-y-auto relative">
      <header className="px-6 py-4 bg-white border-b border-slate-200 sticky top-0 z-10 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/verdesk" className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Campanhas Marketing</h1>
            <p className="text-slate-500">Envio em massa e segmentação pelo WhatsApp</p>
          </div>
        </div>
      ...
      <main className="p-6">
        <div className="max-w-5xl mx-auto space-y-4">
          ...
          {campaigns.map(campaign => (
            <div key={campaign.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow group flex items-start gap-6">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1">
                  <h3 className="text-base font-bold text-slate-900 truncate">{campaign.title}</h3>
                ...
                <p className="text-sm text-slate-500 line-clamp-2 mb-3 max-w-2xl bg-slate-50 p-3 rounded-lg border border-slate-100 font-medium">
                  "{campaign.message}"
                </p>

                <div className="flex items-center gap-4 text-xs font-semibold text-slate-500">
                  <span className="flex items-center gap-1.5 bg-slate-100 px-2 py-1 rounded-md">
                    <Users size={14} className="text-slate-400" /> Audiência: {campaign.targetAudience}
                  </span>
                ...
                {campaign.metrics ? (
                  <div className="flex gap-4 p-3 bg-slate-50 border border-slate-100 rounded-xl">
                    <div className="text-center">
                      <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Entregues</span>
                      <span className="text-sm font-bold text-slate-800">{campaign.metrics.delivered}</span>
                    </div>
                    <div className="w-px bg-slate-200" />
                    <div className="text-center">
                      <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Respostas</span>
      ...
      {isWizardOpen && (
        <>
          ...
          <div className="fixed right-0 top-0 bottom-0 w-[500px] bg-white shadow-2xl z-50 flex flex-col border-l border-slate-200 animate-slide-in">
            <div className="px-6 py-5 border-b border-slate-200 bg-slate-50">
              <h2 className="text-xl font-bold text-slate-900 mb-4">Nova Campanha WhatsApp</h2>
                  ...
                  <div 
                    ...
                    className={`
                      ...
                      ${wizardStep === step ? 'bg-indigo-600 border-indigo-600 text-white' : 
                        wizardStep > step ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-300 text-slate-500'}
                    `}
                  >
              ...
              <div className="flex justify-between text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2 px-1">
                <span>Público</span>
                <span>Mensagem</span>
                <span>Disparo</span>
              </div>
            ...
            <div className="flex-1 p-6 overflow-y-auto custom-scrollbar bg-slate-50/50">
              ...
              {wizardStep === 2 && (
                <div className="space-y-6">
                  ...
                  {/* Preview WA Bubble */}
                  <div className="bg-[#E5DDD5] p-4 rounded-xl relative border border-slate-200">
                    <div className="bg-[#dcf8c6] p-3 rounded-lg rounded-tr-none shadow-sm max-w-[85%] ml-auto text-sm text-[#303030] relative overflow-hidden">
                      ...
                      <div className="text-[10px] text-slate-500 text-right mt-1 font-medium">14:00 <CheckCircle2 size={12} className="inline text-blue-500" /></div>
                    </div>
                  </div>
                </div>
              )}
              ...
              {wizardStep === 3 && (
                <div className="space-y-6">
                  <div className="p-5 border border-slate-200 rounded-xl bg-white shadow-sm space-y-4 text-sm font-medium">
                    <div className="flex justify-between border-b border-slate-100 pb-3">
                      <span className="text-slate-500">Público selecionado</span>
                      <span className="font-bold text-slate-900">Todos os contatos</span>
                    </div>
                    <div className="flex justify-between border-b border-slate-100 pb-3">
                    ...
                    <div>
                      <span className="block text-slate-500 mb-2">Mensagem</span>
                      <p className="p-3 bg-slate-50 rounded-lg text-slate-700 italic border border-slate-100">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-3">Quando disparar?</label>
                    <div className="space-y-2">
                      <label className="flex items-center gap-3 p-3 border border-indigo-500 bg-indigo-50 rounded-xl cursor-pointer ring-1 ring-indigo-500 outline-none">
                        ...
                        <span className="text-sm font-bold text-slate-900">Disparar Agora</span>
                      </label>
                      <label className="flex items-center gap-3 p-3 border border-slate-200 bg-white rounded-xl cursor-pointer hover:bg-slate-50 transition-colors">
                        <input type="radio" name="disparo" className="w-4 h-4 text-indigo-600 focus:ring-indigo-500 border-slate-300" />
                        <span className="text-sm font-bold text-slate-700">Agendar Data e Hora</span>
                      </label>
                    </div>
                  </div>
                </div>
            ...
            <div className="p-5 border-t border-slate-200 bg-white flex justify-between gap-3">
              <button 
              ...
              <div className="flex gap-2">
                {wizardStep > 1 && (
                  <button 
                    ...
                    className="px-4 py-2 text-sm font-bold text-slate-600 border border-slate-200 hover:bg-slate-50 rounded-lg transition-colors"
                  >
                    Voltar
                  </button>
                )}
