...
export function PrescricoesPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--color-bg-card)]">
      <header className="px-6 py-5 bg-[var(--color-bg-card)] border-b border-[var(--color-border)]">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[var(--color-text-muted)] text-sm mt-0.5">Prescrições com assinatura eletrônica e exportação PDF</p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[220px] max-w-sm">
            ...
            <input type="text" placeholder="Buscar por paciente..." value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}
              className="w-full pl-9 pr-4 py-2 text-sm border border-[var(--color-border)] rounded-lg outline-none focus:ring-2 focus:ring-cyan-500/20" />
          </div>
            ...
            {(['todos', 'ativa', 'cancelada'] as const).map(s => (
              <button key={s} onClick={() => { setFilterStatus(s); setPage(1) }}
                className={cn('px-3 py-1.5 rounded-md text-xs font-medium transition-all capitalize',
                  filterStatus === s ? 'bg-[var(--color-bg-card)] text-cyan-600 shadow-sm border border-[var(--color-border)]' : 'text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)]')}>
          ...
          <select value={filterPeriod} onChange={e => { setFilterPeriod(e.target.value as any); setPage(1) }}
            className="px-3 py-2 text-xs font-medium text-[var(--color-text-secondary)] bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-lg outline-none focus:ring-2 focus:ring-cyan-500/20">
        ...
        {isLoading ? (
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            <div className="flex items-center justify-between">
              <p className="text-xs text-[var(--color-text-muted)]">{filtered.length} prescrição(ões){search ? ' encontrada(s)' : ''}</p>
            </div>
            ...
            <div className="space-y-3">
              {paginated.map(p => {
                ...
                return (
                  <div key={p.id} className="bg-[var(--color-bg-card)] rounded-xl border border-[var(--color-border)] shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                    ...
                    <div className="p-4 flex items-center gap-4 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : p.id)}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-[var(--color-text-primary)] text-sm">{p.paciente_nome}</h3>
                        ...
                        <p className="text-xs text-[var(--color-text-muted)] mt-0.5">Dr(a). {p.profissional_nome} · {fmtDate(p.created_at)}</p>
                        {previewText && !isExpanded && (
                          <p className="text-[11px] text-[var(--color-text-muted)] mt-1 truncate">{previewText}</p>
                        )}
                      </div>
                      <ChevronDown className={cn("w-4 h-4 text-[var(--color-text-dim)] transition-transform shrink-0", isExpanded && "rotate-180")} />
                    </div>
                    ...
                    {isExpanded && (
                      <div className="border-t border-[var(--color-border)] animate-fade-in">
                        <div className="p-4 bg-[var(--color-bg-card)]">
                          <pre className="text-xs text-[var(--color-text-secondary)] font-mono whitespace-pre-wrap leading-relaxed">{p.conteudo}</pre>
                        </div>
                        <div className="px-4 py-3 flex items-center justify-between border-t border-[var(--color-border)]">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => handlePrint(p)} className="flex items-center gap-1 px-2.5 py-1.5 text-[10px] font-semibold text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-card-hover)] rounded-lg transition-colors">
                            ...
                            <button onClick={() => setDeleteConfirm(p.id)} className="flex items-center gap-1 px-2.5 py-1.5 text-[10px] font-semibold text-[var(--color-text-muted)] hover:bg-red-50 hover:text-red-500 rounded-lg transition-colors">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-[var(--color-border)]">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                    className="w-8 h-8 flex items-center justify-center rounded-lg border border-[var(--color-border)] hover:bg-[var(--color-bg-card-hover)] disabled:opacity-30 transition-colors text-[var(--color-text-muted)]">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
                    <button key={n} onClick={() => setPage(n)}
                      className={cn("w-8 h-8 flex items-center justify-center rounded-lg text-xs font-semibold transition-colors",
                        page === n ? "bg-cyan-600 text-white" : "border border-[var(--color-border)] text-[var(--color-text-muted)] hover:bg-[var(--color-bg-card-hover)]")}>
                      {n}
                    </button>
                  ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-[var(--color-bg-card)] rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-fade-in">
            <div className="flex items-center justify-between p-6 border-b border-[var(--color-border)]">
              ...
              <button onClick={() => setEditingPresc(null)} className="p-2 hover:bg-[var(--color-bg-card-hover)] rounded-lg"><X className="w-5 h-5 text-[var(--color-text-muted)]" /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="bg-[var(--color-bg-deep)] rounded-lg p-3 text-sm">
                <span className="text-xs font-bold text-[var(--color-text-muted)] uppercase">Paciente:</span>
                <span className="ml-2 font-semibold text-[var(--color-text-secondary)]">{editingPresc.paciente_nome}</span>
              </div>
              <div>
                <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">Conteúdo da Prescrição</label>
                <textarea rows={12} className="w-full p-3 text-sm font-mono border border-[var(--color-border)] rounded-lg outline-none focus:ring-2 focus:ring-cyan-500/20 resize-none"
              ...
              <div className="flex gap-3">
                <button onClick={() => setEditingPresc(null)} className="flex-1 px-4 py-2.5 font-medium text-[var(--color-text-secondary)] bg-[var(--color-bg-card)] hover:bg-[var(--color-border)] rounded-xl transition-colors">Cancelar</button>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-[var(--color-bg-card)] rounded-2xl shadow-xl w-full max-w-sm animate-fade-in">
            <div className="p-6 text-center">
              <div className="w-14 h-14 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <Trash2 size={28} className="text-red-500" />
              </div>
              <h3 className="text-lg font-bold text-[var(--color-text-primary)] mb-1">Excluir prescrição?</h3>
              <p className="text-sm text-[var(--color-text-muted)]">Esta ação é irreversível. A prescrição será removida permanentemente do sistema.</p>
            </div>
            <div className="px-6 pb-6 flex gap-3">
              <button onClick={() => setDeleteConfirm(null)}
                className="flex-1 px-4 py-2.5 font-medium text-[var(--color-text-secondary)] bg-[var(--color-bg-card)] hover:bg-[var(--color-border)] rounded-xl transition-colors">
                Cancelar
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[var(--color-bg-card)] rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col animate-fade-in">
            <div className="bg-gray-800 px-6 py-3 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => handlePrint(viewingPresc)} className="p-2 text-[var(--color-text-muted)] hover:text-white hover:bg-[var(--color-bg-card)]/10 rounded-lg transition-colors" title="Imprimir"><Printer className="w-4 h-4" /></button>
                <button onClick={() => handleDownloadPDF(viewingPresc)} className="p-2 text-[var(--color-text-muted)] hover:text-white hover:bg-[var(--color-bg-card)]/10 rounded-lg transition-colors" title="PDF"><Download className="w-4 h-4" /></button>
                <button onClick={() => setViewingPresc(null)} className="p-2 text-[var(--color-text-muted)] hover:text-white hover:bg-[var(--color-bg-card)]/10 rounded-lg transition-colors"><X className="w-4 h-4" /></button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-8 flex justify-center">
              <div className="bg-[var(--color-bg-card)] w-full max-w-xl shadow-xl rounded-sm border border-[var(--color-border)]" style={{ minHeight: '500px' }}>
                ...
                <div className="px-8 pt-8 pb-4 text-center">
                  <h3 className="text-base font-black text-[var(--color-text-primary)] uppercase tracking-wider">Prescrição Médica</h3>
                  <div className="w-16 h-0.5 bg-cyan-500 mx-auto mt-3" />
                </div>
                <div className="px-8 py-3">
                  <div className="grid grid-cols-3 gap-4 p-4 bg-[var(--color-bg-deep)] rounded-lg text-xs">
                    <div><p className="text-[9px] font-bold text-[var(--color-text-muted)] uppercase">Paciente</p><p className="text-[var(--color-text-secondary)] font-bold mt-1">{viewingPresc.paciente_nome}</p></div>
                    <div><p className="text-[9px] font-bold text-[var(--color-text-muted)] uppercase">Data</p><p className="text-[var(--color-text-secondary)] font-bold mt-1">{fmtDate(viewingPresc.created_at)}</p></div>
                    <div><p className="text-[9px] font-bold text-[var(--color-text-muted)] uppercase">Profissional</p><p className="text-[var(--color-text-secondary)] font-bold mt-1">{viewingPresc.profissional_nome}</p></div>
                  </div>
                </div>
                <div className="px-8 py-4">
                  <pre className="text-sm text-[var(--color-text-secondary)] leading-relaxed whitespace-pre-line font-serif">{viewingPresc.conteudo}</pre>
                </div>
                <div className="px-8 py-6 mt-4 border-t-2 border-dashed border-[var(--color-border)] text-center">
                  <p className="text-[9px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest mb-2">Assinatura Digital</p>
                  ...
                  <p className="text-xs text-[var(--color-text-secondary)] font-semibold">{viewingPresc.profissional_nome}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-[var(--color-bg-card)] rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-fade-in">
            <div className="flex items-center justify-between p-6 border-b border-[var(--color-border)]">
              ...
              <button onClick={() => setShowModal(false)} className="p-2 hover:bg-[var(--color-bg-card-hover)] rounded-lg"><X className="w-5 h-5 text-[var(--color-text-muted)]" /></button>
            </div>
            <form onSubmit={handleCriar} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">Paciente *</label>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl shadow-xl z-10 overflow-hidden">
                      {pacienteOpts.map(p => (
                        <button key={p.id} type="button" onClick={() => { setPacienteSelecionado(p); setPacienteSearch(p.nome); setPacienteOpts([]) }}
                          className="w-full text-left px-4 py-2.5 text-sm hover:bg-cyan-500/5 transition-colors"><span className="font-medium">{p.nome}</span>{p.cpf && <span className="text-[var(--color-text-muted)] ml-2 text-xs">CPF: {p.cpf}</span>}</button>
                      ))}
                    </div>
                  )}
                </div>
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">Conteúdo da Prescrição *</label>
                <textarea rows={12} required className="w-full p-3 text-sm font-mono border border-[var(--color-border)] rounded-lg outline-none focus:ring-2 focus:ring-cyan-500/20 resize-none"
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 px-4 py-2.5 font-medium text-[var(--color-text-secondary)] bg-[var(--color-bg-card)] hover:bg-[var(--color-border)] rounded-xl transition-colors">Cancelar</button>
