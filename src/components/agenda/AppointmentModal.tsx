...
export function AppointmentModal({ isOpen, onClose, onSubmit, initialDate, initialHour }: AppointmentModalProps) {
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('id, nome_completo, especialidade')
      .eq('clinica_id', clinicaId).in('role', ['profissional', 'admin', 'administrador']).eq('ativo', true)
      .then(({ data, error }) => {
  ...
  const searchPacientes = useCallback(async (q: string) => {
    ...
    const { data } = await supabase.from('pacientes').select('id, nome_completo, telefone, convenio')
      .eq('clinica_id', clinicaId).ilike('nome_completo', `%${q}%`).limit(6)
    setPacientesFiltrados((data || []).map((p: any) => ({ id: p.id, nome: p.nome_completo, telefone: p.telefone || '', convenio: p.convenio || '' })))
  }, [clinicaId])
  ...
  const handleSelectPaciente = (pac: PacienteOption & { convenio?: string }) => {
    ...
    setShowPacienteDropdown(false)
    setSelectedConvenio(pac.convenio || null)
  }
          ...
          <div>
            ...
            {selectedConvenio && (
              <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg">
                <span className="text-xs font-semibold text-blue-700">Convênio: {selectedConvenio}</span>
                <span className="text-[10px] text-blue-500">— Verifique cobertura do procedimento e autorização prévia</span>
              </div>
            )}
