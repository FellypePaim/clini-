...
export function ContactContext({ conversation, onPatientLinked }: ContactContextProps) {
  ...
  return (
    <div className="w-80 flex flex-col border-l border-[var(--color-border)] h-full bg-[var(--color-bg-card)] shrink-0 overflow-y-auto custom-scrollbar p-8 animate-fade-in">
       {/* Context Header */}
       <div className="mb-10 text-center flex flex-col items-center">
          <div className="relative inline-block mb-6">
             <Avatar nome={conversation.contato_nome || '?'} size="lg" className="w-24 h-24 text-4xl border-4 border-white shadow-2xl shadow-black/5" />
             {isLead && (
          ...
          <h3 className="text-lg font-black text-[var(--color-text-primary)] border-none uppercase tracking-widest leading-tight">{displayName}</h3>
          <p className="text-[10px] text-[var(--color-text-muted)] font-bold tracking-[0.2em] mt-2 bg-[var(--color-bg-deep)] px-3 py-1 rounded-full">{conversation.contato_telefone}</p>
       </div>

       {/* AI Intent Detection */}
       <div className="mb-10">
          <h4 className="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-[0.3em] mb-4 flex items-center gap-2">
          ...
          {!isLead ? (
            <div className="space-y-3">
              <button
                ...
                className="w-full flex items-center justify-between p-4 bg-[var(--color-bg-deep)] hover:bg-[var(--color-bg-card)] rounded-2xl border border-[var(--color-border)] transition-all group shadow-sm"
              >
                 <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[var(--color-bg-card)] flex items-center justify-center shadow-sm text-cyan-500 group-hover:scale-110 transition-transform">
                       <Activity className="w-5 h-5" />
                    </div>
                    <div className="text-left">
                       <p className="text-[10px] font-black text-[var(--color-text-primary)] uppercase">Ver Prontuario</p>
                       <p className="text-[9px] text-[var(--color-text-muted)] font-medium">Historico Clinico Completo</p>
                    </div>
                 </div>
                 <ArrowRight className="w-4 h-4 text-[var(--color-text-dim)] group-hover:text-cyan-500 transition-colors" />
              </button>
            </div>
          ) : (
       ...
       {/* Quick Actions */}
       <div className="mb-10">
          <h4 className="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-[0.3em] mb-6 flex items-center gap-2">
       ...
       {showModal && (
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowModal(false)}>
           <div className="bg-[var(--color-bg-card)] rounded-3xl shadow-2xl w-full max-w-md mx-4 p-8 space-y-6 animate-fade-in" onClick={e => e.stopPropagation()}>
             <div className="flex items-center justify-between">
               <h2 className="text-lg font-black text-[var(--color-text-primary)]">Cadastrar Paciente</h2>
               <button onClick={() => setShowModal(false)} className="p-2 hover:bg-[var(--color-bg-card-hover)] rounded-xl"><X className="w-5 h-5 text-[var(--color-text-muted)]" /></button>
             </div>

             <div className="space-y-4">
               <div>
                 <label className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">Nome Completo *</label>
                 <input value={form.nome_completo} onChange={e => setForm(f => ({ ...f, nome_completo: e.target.value }))}
                   className="w-full mt-1 px-4 py-3 border border-[var(--color-border)] rounded-xl text-sm focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500 outline-none" />
               </div>
               <div className="grid grid-cols-2 gap-3">
                 <div>
                   <label className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">Telefone</label>
                   <input value={form.telefone} onChange={e => setForm(f => ({ ...f, telefone: e.target.value }))}
                     className="w-full mt-1 px-4 py-3 border border-[var(--color-border)] rounded-xl text-sm focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500 outline-none" />
                 </div>
                 <div>
                   <label className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">WhatsApp</label>
               ...
               <div className="grid grid-cols-2 gap-3">
                 <div>
                   <label className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">E-mail</label>
                   <input value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} type="email"
                     className="w-full mt-1 px-4 py-3 border border-[var(--color-border)] rounded-xl text-sm focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500 outline-none" />
                 </div>
                 <div>
                   <label className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">CPF</label>
               ...
               <div className="grid grid-cols-2 gap-3">
                 <div>
                   <label className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">Nascimento</label>
                   <input value={form.data_nascimento} onChange={e => setForm(f => ({ ...f, data_nascimento: e.target.value }))} type="date"
                     className="w-full mt-1 px-4 py-3 border border-[var(--color-border)] rounded-xl text-sm focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500 outline-none" />
                 </div>
                 <div>
                   <label className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">Genero</label>
                   <select value={form.genero} onChange={e => setForm(f => ({ ...f, genero: e.target.value }))}
                     className="w-full mt-1 px-4 py-3 border border-[var(--color-border)] rounded-xl text-sm focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500 outline-none bg-[var(--color-bg-card)]">
...
function ActionButton({ icon, label, color, onClick }: { icon: React.ReactNode, label: string, color: string, onClick?: () => void }) {
  const colors: Record<string, string> = {
    ...
    gray: "bg-[var(--color-bg-deep)] text-[var(--color-text-secondary)] hover:bg-gray-600 hover:text-white border border-[var(--color-border)] hover:border-gray-600"
  }
